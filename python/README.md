...
```sh
pipx install miss-u-more      # or: uv tool install miss-u-more
uvx miss-u-more               # or run it without installing
```

Requires Python 3.9+. No dependencies — the standard library is enough.
...
install, and it runs wherever Python does. If you would rather have a single
binary, `brew install glimpsed-io/tap/miss-u-more` and `scoop install miss-u-more` ship one.
